...
class TrkDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        template_bbox_tensor = torch.tensor([template_bbox.x1, template_bbox.y1,
                                           template_bbox.x2 - template_bbox.x1,
                                           template_bbox.y2 - template_bbox.y1])
        ...
        #         search_bbox, cfg.TRAIN.OUTPUT_SIZE, neg)
        template = template.transpose((2, 0, 1)).astype(np.float32)
        search = search.transpose((2, 0, 1)).astype(np.float32)

        # 通过高斯噪声，生成多个bbox, 输入和输出都是tensor
        # search_bbox_tensor, proposals [x,y,w,h]
        search_bbox_tensor = torch.tensor([search_bbox.x1, search_bbox.y1,
                                           search_bbox.x2 - search_bbox.x1,
                                           search_bbox.y2 - search_bbox.y1])
        proposals, gt_iou = self._generate_proposals(search_bbox_tensor)

        return {
                'template': template,
                'search': search,
                'search_bbox': search_bbox_tensor,
                'template_bbox':  template_bbox_tensor,
                'proposals_bbox': proposals,
                'proposals_iou': gt_iou
                }
