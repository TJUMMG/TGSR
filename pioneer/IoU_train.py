...
parser.add_argument('--save_model_path', type=str,
                    default='/media/HardDisk_new/wh/second_code/pysot-master_0713/Albation_model/ioupred_dr_correct/',
                    help='')
...
def train(train_loader, model, optimizer, lr_scheduler, tb_writer):
    ...
    for idx, data in enumerate(train_loader):
        if epoch != idx // num_per_epoch + start_epoch:
            ...
            if get_rank() == 0:
                torch.save(model.module.iou_pred.state_dict(),
                           args.save_model_path + '/ipn_e%d.pth' % (epoch))
        ...
        outputs, pred_iou = model.forward_iou(data)
        loss = outputs['loss']
        ...
        if rank == 0:
            ...
            if (idx+1) % cfg.TRAIN.PRINT_FREQ == 0:
                ...
                print('pred:', pred_iou.detach().cpu().numpy()[0,1,:])
                print('true:', data['proposals_iou'].cpu().numpy()[1,:])
...
def main():
    ...
    # model.iou_pred.load_state_dict(
    #     torch.load('/media/HardDisk_new/wh/second_code/pysot-master_0713/snapshot_test/iounet_20210623_4.pth'))

    model.eval()
    model.iou_pred.train()
    ...
    optimizer = torch.optim.Adam(model.iou_pred.parameters(), lr=1e-4)  # 1e-3
