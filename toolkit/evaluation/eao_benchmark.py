...
class EAOBenchmark:
    ...
    def _calculate_eao(self, tracker_name, tags):
        ...
        for tag in tags:
            ...
            for name, traj_len, failures, overlaps in zip(video_names, gt_traj_length,
                ...
                if len(failures) > 0:
                    points = [x+self.skipping for x in failures if
                            x+self.skipping <= len(overlaps)]   # self.skipping = 5
                    points.insert(0, 0)
            ...
            # 观察fragment在[self.low-1:self.high-1+1]中的情况
            # see = fragments[:, self.low - 1:self.high - 1 + 1]
            # n = np.zeros([see.shape[0]])
            # reco = []
            # for i_ in range(see.shape[0]):
            #     # if np.any(np.isnan(see[i_,:])):
            #     #     continue
            #     for w__,w_ in enumerate(see[i_, :]):
            #
            #         if w__ == 0:
            #             continue
            #         else:
            #             if np.isnan(w_) and see[i_, w__ - 1]==0.:
            #                 reco.append([i_, w__])
            #                 continue
            #
            #         n[i_] += np.where(w_ == 0.0, 1, 0)
            # n_num = np.sum(n)
