...
def calculate_expected_overlap(fragments, fweights):
    ...
    for i in range(1, max_len):
        mask = np.logical_not(np.isnan(fragments[:, i])) # 寻找fragments前i列数值不是nan的坐标
        if np.any(mask): # 将前i列值非nan的求平均，即求前i帧的平均overlap
            fragment = fragments[mask, 1:i+1]
            seq_mean = np.sum(fragment, 1) / fragment.shape[1]  # 对单个视频的前i帧
            expected_overlaps[i] = np.sum(seq_mean *
                fweights[mask]) / np.sum(fweights[mask])    # 对所有视频的前i帧
